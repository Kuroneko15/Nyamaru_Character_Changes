...
{
	name = "L4D2 Special Infected Spawn",
	author = "Kuroneko15",
	description = "Chỉnh số lượng và thơi gian sinh sản SI", 
	version = "1.0",
}
...
{
	Init();
	g_cvAdminImmunity = CreateConVar("l4d2_sis_admin", "1", "Admin join infected team (versus only).");
	g_cvBlockOtherRespawn = CreateConVar("l4d2_sis_block_map_respawn", "1", "Block infected spawned by map z_spawn_old command.");

	g_cvSpawnTime = 				CreateConVar("l4d2_spawn_dead_time", 		"45", "Spawn time");

	g_cvMaxSpecialLimit =			CreateConVar("l4d2_sis_max_si_limit",		"5", "Max SI limit");
	g_cvSpecialLimit[SMOKER] =		CreateConVar("l4d2_sis_smoker_limit",		"1", "Smoker spawn limit.");
	g_cvSpecialLimit[BOOMER] =		CreateConVar("l4d2_sis_boomer_limit",		"1", "Boomer spawn limit.");
	g_cvSpecialLimit[HUNTER] =		CreateConVar("l4d2_sis_hunter_limit",		"1", "Hunter spawn limit.");
	g_cvSpecialLimit[SPITTER] =		CreateConVar("l4d2_sis_spitter_limit",		"1", "Spitter spawn limit.");
	g_cvSpecialLimit[JOCKEY] =		CreateConVar("l4d2_sis_jockey_limit",		"1", "Jockey spawn limit.");
	g_cvSpecialLimit[CHARGER] =		CreateConVar("l4d2_sis_charger_limit",		"1", "Charger limit.");
	g_cvSpecialLimit[TANK] =		CreateConVar("l4d2_sis_tank_limit",			"1", "Tank limit.");
	...
	RegConsoleCmd("sm_joinSI", Cmd_JoinTeamSI);
	RegConsoleCmd("sm_jointank", Cmd_JoinTank);
...
void Init()
{
	...
	strcopy(sBuffer, sizeof(sBuffer), "l4d2_si_spawn");
	...
	SetupPatch(hGameData, "CTerrorPlayer::UpdateZombieFrustration::SkipUselessCode");
...
Action SpawnSI_Timer(Handle timer, int userid)
{
	...
	if (g_bLeftSafeArea && client > 0 && IsClientInGame(client) && GetClientTeam(client) == TEAM_INF && !IsPlayerAlive(client) && !IsFakeClient(client))
	{
		if (g_iSpawnCountDown[client] <= 0)
		{
			...
			{
				CPrintToChat(client, "{lightgreen}[Report] {green}Failed to spawn, please try again.");
		...
		PrintHintText(client, "Đang chờ hồi sinh sau %i giây", g_iSpawnCountDown[client]--);
...
Action Cmd_JoinTeamSI(int client, int args)
{
	...
	if (GetZombiePlayerTotal() < g_iMaxSpecialLimit)
		...
		PrintHintText(client, "The limit of infected team has been reached maximum.");
...
Action Cmd_JoinTank(int client, int args)
{
	...
	if (RemoveJoinTank(userid))
		CPrintToChat(client, "{lightgreen}[Tank] {green}You have exited from taking over the {olive}Tank.");
	else
	{
		g_aJoinTankList.Push(userid);
		CPrintToChat(client, "{lightgreen}[Tank] {green}Tank has been you control.");
	}
...
void Event_PlayerTeam(Event event, const char[] name, bool dontBroadcast)
{
	...
	switch (newTeam)
	{
		case TEAM_INF:
		{
			...
			{
				mp_gamemode.ReplicateToClient(client, "versus");
				CPrintToChat(client, "{blue}[Command] {green}Type {olive}!JoinSI {green}to join team special infected. {olive}!joinTank to takeover Tank control.");
...
void Event_PlayerReplacedBot(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (player > 0 && IsClientInGame(player) && GetClientTeam(player) == TEAM_INF && GetZombieClass(player) == TANK && IsPlayerAlive(player))
		CPrintToChatAll("{blue}[Tank] {olive}Tank {default}has been {olive}%N {default}control.", player);
}
